...
class ReportAction(models.Model):
    """
    Extends ir.actions.report to add 'xlsx' as a report type and implement
    the rendering & attachment-saving hooks for Excel outputs.
    """
    ...
    def _render_xlsx(self, report_ref, docids, data):
        """
        Renders an XLSX report by calling the target abstract report model:
        'report.<report_name>'. Returns (bytes, "xlsx") as expected by Odoo.
        """
    ...
    def _get_report_from_name(self, report_name):
        """
        Let Odoo find non-QWeb 'xlsx' reports by report_name.
        Falls back to base implementation first.
        """
    ...
    def save_xlsx_report_attachment(self, docids, report_contents):
        """
        Save result as an ir.attachment when 'attachment' is configured on the
        ir.actions.report and exactly one document is targeted.
        """
