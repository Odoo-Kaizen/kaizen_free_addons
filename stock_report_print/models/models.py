...
class ProductProduct(models.Model):
    ...
    def _compute_totals_purchases(self):
        ...
        groups = self.env['purchase.order.line'].read_group(domain, ['product_id', 'product_uom_qty'], ['product_id'])
        purchased_data = {
            g['product_id'][0]: g['product_uom_qty']
            for g in groups if g['product_id']
        }
        for product in self:
            if not product.id:
    ...
    def _compute_totals_sales(self):
        ...
        groups = self.env['sale.report'].read_group(domain, ['product_id', 'product_uom_qty'], ['product_id'])
        for group in groups:
